...
## 9. 빌드/호환 유의사항
- 현재 venv는 **Python 3.14.0**. `agent-framework`는 preview라 3.14용 휠이 없거나 빌드 의존성 충돌 가능.
  - 설치 실패 시 **Python 3.12/3.13로 venv 재생성** 권장(`py -3.12 -m venv .venv`).
- 설치 검증: `python -c "import agent_framework; print(agent_framework.__version__)"`.
